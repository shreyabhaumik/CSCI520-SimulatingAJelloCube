...
void saveScreenshot(int windowWidth, int windowHeight, char *filename)
{
  ...
  w1 = windowWidth*2;
  h1 = windowHeight*2;
  Pic * in = pic_alloc(w1, h1, 3, NULL);

  printf("File to save to: %s\n", filename);

  for (int i=h1-1; i>=0; i--) 
  {
    glReadPixels(0, h1-i-1, w1, 1, GL_RGB, GL_UNSIGNED_BYTE,
...
void mouseMotionDrag(int x, int y)
{
  ...
  if (g_iLeftMouseButton)
  {
    ...
    glGetIntegerv(GL_VIEWPORT, vp); // To get the x, y, width and height window coordinates of the current viewport
    // To compute the 3D coordinates of initial mouse position
    pMAKE(dragOrigin[0], vp[3] - dragOrigin[1] + 1, 1, initialPos);
    gluUnProject(initialPos.x, initialPos.y, initialPos.z, mvMatrix, projMatrix, vp, &(initialPos.x), &(initialPos.y), &(initialPos.z));
    // To compute the 3D coordinates of final mouse position
    pMAKE(x, vp[3] - y + 1, 1, finalPos);
    gluUnProject(finalPos.x, finalPos.y, finalPos.z, mvMatrix, projMatrix, vp, &(finalPos.x), &(finalPos.y), &(finalPos.z));
    pDIFFERENCE(initialPos,finalPos,mousedragForce);
...
void keyboardFunc (unsigned char key, int x, int y)
{
  switch (key)
  {
    ...
    case 'w': // To toggle between viewing red and blue environments(skybox)
      skyworld = 1 - skyworld;
      break;
  }
}
